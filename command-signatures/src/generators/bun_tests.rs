...
use super::common::get_scripts_generator;

#[cfg(feature = "embed-signatures")]
fn uses_generator(argument: &warp_completion_metadata::Argument, generator_name: &str) -> bool {
...
fn test_bun_remove_argument_completes_installed_dependencies() {
    ...
    for name in ["rm", "remove"] {
        ...
        let package = remove
            .arguments()
            .first()
            .unwrap_or_else(|| panic!("bun {name} should accept a positional package argument"));
...
fn test_bun_registers_the_generators_its_spec_references() {
    let (command, data): (String, DynamicCompletionData) = super::bun::generator().into();
