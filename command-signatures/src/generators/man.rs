...
pub fn generator() -> CommandSignatureGenerators {
    CommandSignatureGenerators::new("man")
        .add_generator(
            "sections_remaining",
            Generator::command_from_tokens(
                super::fig_token::man_sections_remaining,
                super::output_parsers::lines,
            ),
        )
        .add_generator(
            "list_man_pages",
                ...
                |tokens, _, _| {
                    ...
                    CommandBuilder::pipe(
                        CommandBuilder::single_command(format!(
                            "ls -1 $(man -w | sed 's#:#/man{} #g')",
                            section_glob
                        )),
                        CommandBuilder::pipe(
                            CommandBuilder::pipe(
                                CommandBuilder::single_command("cut -f 1 -d ."),
                                CommandBuilder::single_command("sort"),
                            ),
                            CommandBuilder::single_command("uniq"),
                        ),
                    )
                },
                |output| {
