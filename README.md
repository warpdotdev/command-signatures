...
Nested `loadSpec` targets live in `./command-signatures/json/autogenerated/load_spec_targets/{aws,gcloud}/*.json` and are looked up as `aws/<name>` / `gcloud/<name>`. Refresh them from a pinned upstream autocomplete checkout with:
